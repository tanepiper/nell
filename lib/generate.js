...
var generateOutputDetails = function(name, site_path, type) {
  ...
  var extension = name.indexOf('.md');
  var folder = name.substring(11, extension);

  return {
    output_path: path.join(site_path, 'output', type + 's', year, month, day, folder),
    link_path: path.join('posts', year, month, day, folder),
    named_path: path.join(site_path, 'output', type + 's', year, month, day, folder, 'index.html')
  };
};
